...
const Offer = ({products}) => {
  return (
    <div className="max-w-screen-xl mx-auto my-12">
      <h2 className="font-bold text-slate-700 text-center sm:text-left text-2xl sm:text-5xl font-serif underline mt-8 mb-8">Limited Time Offers</h2>
      <div className="flex flex-col sm:flex-row flex-wrap ">
        
        {products.map(product => (
          <Link to={`/products/${product.id}`}>
          <div key={product.id} className="relative w-[300px] rounded border-2 p-4 m-2">
            <button className='absolute border-2 bg-green-700 shadow-md px-3 py-2 rounded text-white font-semibold'>Sale!</button>
